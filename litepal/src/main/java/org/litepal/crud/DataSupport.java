 ...
 * @since 1.1
 */
public class DataSupport extends BmobObject{
	 ...
	 * @return The number of rows affected. Including cascade delete rows.
	 */
	public synchronized int deletes() {
		SQLiteDatabase db = Connector.getDatabase();
		db.beginTransaction();
		try {
			DeleteHandler deleteHandler = new DeleteHandler(db);
    ...
    /**
     * Basically same as {@link #deletes()} but pending to a new thread for executing.
     *
     * @return A UpdateOrDeleteExecutor instance.
     */
    public UpdateOrDeleteExecutor deleteAsync() {
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                synchronized (DataSupport.class) {
                    final int rowsAffected = deletes();
                    if (executor.getListener() != null) {
                        LitePal.getHandler().post(new Runnable() {
                            @Override
                            public void run() {
	 ...
	 */
	public synchronized boolean saves() {
		try {
			saveThrows();
			return true;
		} catch (Exception e) {
    ...
    /**
     * Basically same as {@link #saves()} but pending to a new thread for executing.
     *
     * @return A SaveExecutor instance.
     */
